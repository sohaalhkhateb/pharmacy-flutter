import 'package:flutter/material.dart';
import 'package:flutter_bloc/flutter_bloc.dart';
import 'package:my_pharmacy/layout/pharmacy_app/product_details/cubit/cubit.dart';
import 'package:my_pharmacy/layout/pharmacy_app/product_details/cubit/states.dart';
import 'package:my_pharmacy/shared/components/components.dart';


class ProductsSearchScreen extends StatelessWidget {
  @override
  Widget build(BuildContext context)
  {
    var searchController = TextEditingController();

    return BlocProvider(
      create: (context) => ProductDetailsCubit()..getProducts(),
      child: BlocConsumer<ProductDetailsCubit, ProductStates>(
        listener: (context, state) {},
        builder: (context, state)
        {
          var list = ProductDetailsCubit.get(context).search;
          return Scaffold(
            appBar: AppBar(),
            body: Column(
              children:
              [
                Padding(
                  padding: const EdgeInsets.all(20.0),
                  child: defultTextField(
                    controller: searchController,
                    type: TextInputType.text,
                    onChange: (value)
                    {
                      ProductDetailsCubit.get(context).getSearch(value);
                    },
                    validate: (String? value)
                    {
                      if(value == null || value.isEmpty)
                      {
                        return'search must not be empty';
                      }
                      return null;
                    },
                    label: 'Search',
                    prefix: Icons.search,
                  ),
                ),
                Expanded(child: productBuilder(list, context, true,isSearch: true,))
              ],
            ),
          );
        },
      ),
    );
  }
}
